...
# (녹화 시작 시각(초), 자막 문장) — 실행 화면 진행에 맞춰 배치
BLOCKS = [
    (0, "프롬프트 난이도 인지 기반 경량 LLM 라우터입니다. 프롬프트 내용만 보고 세 후보 "
        "모델 중 하나를 선택합니다. 모델을 호출하거나 답변을 비교할 수 없고, 예산을 "
        "넘기면 해당 등급은 영 점 처리됩니다."),
    (14, "먼저 라우팅을 실행합니다. 공개 검증 자료 팔백팔십 문항을 프리미엄 등급으로 "
         "처리합니다. 사 점 이 초가 걸렸고, 제한은 등급당 구십 초입니다."),
    (30, "선택 결과를 확인합니다. 팔백팔십 문항 전부에 모델이 하나씩 배정되었고, "
         "경량 모델과 중형 모델, 추론 모델의 분포를 볼 수 있습니다."),
    (44, "공식 채점기로 검증합니다. 최종 점수 영 점 칠일육칠, 세 등급 모두 예산 안에 "
         "있습니다. 예산 사용률은 각각 팔십삼, 팔십, 육십오 퍼센트입니다."),
    (62, "직접 만든 자체 점검 도구입니다. 문항 아이디와 입력 순서를 바꿔 재실행해도 "
         "결과가 같은지, 같은 입력에 같은 출력이 나오는지, 엣지 케이스에서 죽지 않는지, "
         "실행 시간과 예산 여유가 충분한지를 한 번에 확인합니다."),
    (86, "열 항목 모두 통과입니다. 순서 불변성은 사백 문항에서 불일치 영 건, 실행 시간은 "
         "한도의 오 퍼센트, 예산은 세 등급 모두 여유를 남기고 통과했습니다."),
    (104, "마지막으로 실제 제출 이미지를 공식 자원 한도에서 실행합니다. 네트워크 차단, "
          "읽기 전용 루트, 씨피유 두 개, 메모리 이 기가바이트 조건입니다. 레지스트리에서 "
          "내려받은 이미지를 다이제스트로 지정해 실행합니다."),
    (128, "정상 종료되었습니다. 학습은 사이킷런으로 하고, 실행 이미지에는 넘파이 하나만 "
...
def main() -> int:
    ...
    for i in targets:
        ...
        total += seconds
        print(f"  n{i:02d}  {start:>4}s 시작  {seconds:5.1f}초  {text[:30]}…")
    ...
    for i, (start, text) in enumerate(BLOCKS):
        ...
        for piece, weight in zip(pieces, weights):
            ...
            entries.append((cursor, cursor + length, wrap(piece)))
            cursor += length
