...
# 편집본(build/rec/cut.mp4) 기준 배치.
# start는 화면에서 그 일이 실제로 벌어지는 시각이다. 녹화를 잘라 붙였으므로
# 아래 주석의 화면 상황과 반드시 함께 고쳐야 한다.
#
#   0.0 인트로 · 2.0 1단계 헤더 · 11.0 라우팅 결과 · 13.5~16 모델 분포
#   16.0~22 결과 화면 유지 · 22.0 2단계 헤더 · 29.5 채점 결과
#   32.5 3단계 헤더 · 37.0 점검 실행 · 41.8~51.8 진행 중
#   53.0 점검 결과 · 55.8~60.8 결과 유지 · 61.0 4단계 헤더 · 65 컨테이너 명령
#   68.8~76.8 실행 중 · 79.0 컨테이너 결과 · 83.8~86.8 마무리
REC_END = 87.5

# (시각, TTS 낭독문, 자막문, 코드 위치 배지)
# TTS는 숫자를 한글로 적어야 자연스럽게 읽고, 자막은 숫자 그대로 보여야 읽기 쉽다.
# 배지는 그 설명이 어느 코드에서 나온 동작인지 화면 오른쪽 위에 띄운다.
BLOCKS = [
    (0.0,
     ...
     None),
    (10.5,
     "먼저 라우팅입니다. 팔백팔십 문항을 사 점 이 초에 처리했습니다.",
     "먼저 라우팅입니다. 880문항을 4.2초에 처리했습니다.",
     ("src/routerx/router.py", "route()", "TF-IDF·밀집 특징 → 품질·비용 예측")),
    (16.9,
     "문항마다 세 모델 중 하나가 배정되고, 그 분포가 함께 나옵니다.",
     "문항마다 세 모델 중 하나가 배정되고, 그 분포가 함께 나옵니다.",
     ("src/routerx/policy.py", "select_batch()", "이득/비용 비로 예산 배분")),
    (27.0,
     "공식 채점기 결과입니다. 최종 점수 영 점 칠일육칠, 세 등급 모두 예산 안입니다.",
     "공식 채점기 결과입니다. 최종 점수 0.7167, 세 등급 모두 예산 안입니다.",
     ("공식 채점 도구", "ossp_router self-check", "점수·예산 준수 판정")),
    (35.6,
     "다음은 자체 점검입니다. 입력 순서를 바꿔도 결과가 같은지, 엣지 케이스에서 죽지 "
     "않는지, 실행 시간과 예산 여유가 충분한지를 한 번에 확인합니다.",
     "다음은 자체 점검입니다. 입력 순서를 바꿔도 결과가 같은지, 엣지 케이스에서 죽지 "
     "않는지, 실행 시간과 예산 여유가 충분한지를 한 번에 확인합니다.",
     ("src/routerx/audit.py", "main()", "순서 불변·결정성·엣지·성능 10항목")),
    (53.0,
     "열 항목 모두 통과입니다. 순서 불변성 불일치 영 건, 실행 시간은 한도의 오 "
     "퍼센트입니다.",
     "10항목 모두 통과입니다. 순서 불변성 불일치 0건, 실행 시간은 한도의 5%입니다.",
     ("src/routerx/policy.py", "_group_rows()", "같은 프롬프트를 한 묶음으로 처리")),
    (61.5,
     "마지막으로 제출 이미지를 공식 한도에서 실행합니다. 네트워크 차단, 읽기 전용 "
     "루트, 씨피유 두 코어, 메모리 이 기가바이트입니다.",
     "마지막으로 제출 이미지를 공식 한도에서 실행합니다. 네트워크 차단, 읽기 전용 "
     "루트, CPU 2코어, 메모리 2GiB입니다.",
     ("Dockerfile", "linux/arm64", "런타임 의존성 NumPy 하나")),
    (79.0,
     "정상 종료되었습니다. 학습은 사이킷런으로 하고, 실행 이미지에는 넘파이 하나만 "
     "넣었습니다.",
     "정상 종료되었습니다. 학습은 scikit-learn으로 하고, 실행 이미지에는 NumPy "
     "하나만 넣었습니다.",
     ("src/routerx/features.py", "tfidf_row()", "학습과 같은 결과를 NumPy만으로 재현")),
]
...
def main() -> int:
    ...
    used = None
    for i in targets:
        start, say = BLOCKS[i][0], BLOCKS[i][1]
        pcm, used = synth(say)
        ...
        total += seconds
        print(f"  n{i:02d}  {start:>5.1f}s 시작  {seconds:5.1f}초  {say[:30]}…")
